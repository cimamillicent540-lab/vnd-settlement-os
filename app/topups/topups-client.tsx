...
type TopupRow={id:string;executionDate:string;sequence:number;usdt:string;grossVnd:string;remainingVnd:string;rate:string;precision:string;channel:string;status:string;matchStatus:string;ledgerTreatment:string;settleableIncreaseVnd:string};

export function TopupsClient({initialRows}:{initialRows:TopupRow[]}){
  const [rows,setRows]=useState<TopupRow[]>(initialRows);
  ...
  async function submit(formData:FormData){
    ...
    try{
      ...
      setRows(current=>[{id:body.data!.id,executionDate:date,sequence:body.data!.sequence_within_date,usdt,grossVnd:vnd,remainingVnd:vnd,rate:body.data!.calculated_rate,precision,channel:String(formData.get("channel")||"Manual"),status:"PENDING",matchStatus:"PENDING_REVIEW",ledgerTreatment:"PENDING_REVIEW",settleableIncreaseVnd:"0"},...current]);
      setShowForm(false);
  ...
  return <>
    ...
    <div className="kpi-grid"><KpiCard label="累计 USDT 成本" value={formatUsdt(totals.totalUsdt)} note={<><span className="trend-up">{rows.length} 个批次</span><span>· 数据库记录</span></>} icon={CircleDollarSign}/><KpiCard label="累计 VND 到账" value={formatVnd(totals.totalVnd)} note={<span>净到账金额 · 无额外费用</span>} icon={Landmark} color="#0f9f78"/><KpiCard label="加权平均执行汇率" value={formatRate(totals.weightedAverageRate,10)} note={<span>VND / USDT · 高精度核算</span>} icon={TrendingUp} color="#6f4bb7"/><KpiCard label="补U桶剩余 VND" value={formatVnd(totals.remaining)} note={<span>数据库当前剩余本金</span>} icon={WalletCards} color="#dc8b16"/></div>
    ...
    <Card><CardHeader><div><h2 className="panel-title">补U批次台账</h2><div className="panel-subtitle">原始到账全额保留；可结算增加为 50%</div></div><Badge variant="blue">UTC</Badge></CardHeader><div className="toolbar"><div className="search"><Search size={14}/><input placeholder="搜索批次、通道…"/></div><select className="select"><option>全部审批状态</option><option>已审批</option><option>待审批</option></select><select className="select"><option>全部时间精度</option><option>DATE_ONLY</option><option>EXACT</option></select></div><div className="table-wrap"><table><thead><tr><th>批次 ID</th><th>执行日期</th><th>通道</th><th style={{textAlign:"right"}}>USDT 成本</th><th style={{textAlign:"right"}}>Gross 到账</th><th style={{textAlign:"right"}}>Settleable 增加</th><th>Account History</th><th>账本处理</th><th>状态</th></tr></thead><tbody>{rows.map(row=><tr key={row.id}><td className="mono">{row.id}</td><td>{row.executionDate} · #{row.sequence}</td><td>{row.channel}</td><td className="money">{formatUsdt(row.usdt)}</td><td className="money">{formatVnd(row.grossVnd)}</td><td className="money">{formatVnd(row.settleableIncreaseVnd)}</td><td><Badge variant={row.matchStatus.startsWith("NOT_")?"amber":"green"}>{row.matchStatus}</Badge></td><td className="mono">{row.ledgerTreatment}</td><td><Badge variant={row.status==="APPROVED"?"green":"amber"}>{row.status==="APPROVED"?"已审批":"待审批"}</Badge></td></tr>)}</tbody></table></div><Pagination total={rows.length} label="个批次"/></Card>
