...
export async function POST(request:Request){
  ...
  const url=process.env.NEXT_PUBLIC_SUPABASE_URL; const key=process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY; const authorization=request.headers.get("authorization");
  ...
  const records=input.rows.map(row=>{
    ...
    if(input.sourceType==="PAYIN"){
      ...
      return {...base,payin_amount_vnd:amount,target_amount_vnd:numeric(row.data.target_amount_vnd),imported_transaction_fee_vnd:numeric(row.data.imported_transaction_fee_vnd),expected_fee_rate:"0.008",expected_fee_revenue_vnd:economics.expectedFeeRevenueVnd,fee_validation_difference_vnd:validation?.differenceVnd??null,fee_validation_status:validation?.status??"MISSING",upstream_success_fee_vnd:"2500",upstream_failure_fee_vnd:"0",upstream_fee_applied_vnd:economics.upstreamFeeAppliedVnd,net_fee_contribution_vnd:economics.netFeeContributionVnd,pool_inflow_vnd:row.data.status==="SUCCESS"?numeric(row.data.target_amount_vnd):null,pool_inflow_status:row.data.status==="SUCCESS"&&row.data.target_amount_vnd?"CONFIRMED":row.data.status==="SUCCESS"?"PENDING_CONFIRMATION":"NOT_APPLICABLE",event_time_confidence:row.data.completed_at?"HIGH":"MEDIUM"};
    }
