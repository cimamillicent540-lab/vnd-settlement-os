...
export async function POST(request:Request){
 ...
 const url=normalizeSupabaseUrl(process.env.NEXT_PUBLIC_SUPABASE_URL);const key=process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY??process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;const authorization=request.headers.get("authorization");if(!url||!key)return NextResponse.json({message:"Supabase 环境变量未配置。"},{status:503});if(!authorization)return NextResponse.json({message:"需要已认证的内部账号。"},{status:401});
