...
type ImportBatchRow={id:string;type:string;file:string;rows:number;valid:number;invalid:number;duplicate:number;status:string;at:string};
export function ImportsClient({initialBatches}:{initialBatches:ImportBatchRow[]}){
  ...
  return <>
    ...
    <Card><CardHeader><div><h2 className="panel-title">导入批次</h2><div className="panel-subtitle">文件哈希与订单行哈希双重防重复</div></div><Badge variant="blue">真实数据</Badge></CardHeader><div className="toolbar"><div className="search"><Search size={14}/><input placeholder="搜索批次、文件名…"/></div><select className="select"><option>全部来源</option><option>PAYIN</option><option>PAYOUT</option><option>ACCOUNT_HISTORY</option></select><span className="toolbar-spacer"/></div><div className="table-wrap"><table><thead><tr><th>批次 ID</th><th>类型</th><th>原始文件</th><th>导入时间</th><th>总行数</th><th>有效</th><th>无效</th><th>重复</th><th>状态</th></tr></thead><tbody>{initialBatches.map(batch=><tr key={batch.id}><td className="mono">{batch.id}</td><td><Badge variant={batch.type==="PAYIN"?"blue":"violet"}>{batch.type}</Badge></td><td>{batch.file}</td><td className="muted">{batch.at}</td><td>{batch.rows.toLocaleString()}</td><td style={{color:"#087f5b"}}>{batch.valid.toLocaleString()}</td><td style={{color:batch.invalid?"#b82f3a":undefined}}>{batch.invalid}</td><td style={{color:batch.duplicate?"#a96506":undefined}}>{batch.duplicate}</td><td><Badge variant={batch.status==="COMPLETED"?"green":"amber"}>{batch.status}</Badge></td></tr>)}</tbody></table></div><Pagination total={initialBatches.length}/></Card>
  </>;
}
