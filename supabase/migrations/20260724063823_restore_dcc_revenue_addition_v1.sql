-- Registered as restore_dcc_revenue_addition_v1.
-- Restore signed DCC addition without mutating prior immutable Shadow runs.
-- Correct formula:
-- total_company_revenue_usdt =
--   merchant_fee_revenue_usdt + signed dcc_revenue_usdt.

create table if not exists public.shadow_pricing_run_supersessions (
  id uuid primary key default gen_random_uuid(),
  superseded_run_id uuid not null unique
    references public.shadow_pricing_runs(id),
  replacement_run_id uuid not null
    references public.shadow_pricing_runs(id),
  effective_status text not null default 'SUPERSEDED'
    check (effective_status = 'SUPERSEDED'),
  reason text not null,
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  check (superseded_run_id <> replacement_run_id)
);
create index if not exists shadow_run_supersessions_replacement_idx
  on public.shadow_pricing_run_supersessions(replacement_run_id);

alter table public.shadow_pricing_run_supersessions enable row level security;
drop policy if exists read_authenticated
  on public.shadow_pricing_run_supersessions;
create policy read_authenticated
on public.shadow_pricing_run_supersessions
for select to authenticated using (true);
grant select on public.shadow_pricing_run_supersessions to authenticated;
grant all on public.shadow_pricing_run_supersessions to service_role;

drop trigger if exists shadow_run_supersessions_immutable
  on public.shadow_pricing_run_supersessions;
create trigger shadow_run_supersessions_immutable
before update or delete on public.shadow_pricing_run_supersessions
for each row execute function private.reject_shadow_pricing_mutation();

create or replace view public.shadow_pricing_run_effective_status
...
select
  ...
  run.run_type,
  run.rules_version,
  run.status as recorded_status,
  coalesce(supersession.effective_status, run.status) as effective_status,
  run.shadow_mode,
  run.created_at,
  supersession.replacement_run_id,
  supersession.reason as supersession_reason,
  supersession.created_at as superseded_at
from public.shadow_pricing_runs run
left join public.shadow_pricing_run_supersessions supersession
  on supersession.superseded_run_id = run.id;
grant select on public.shadow_pricing_run_effective_status to authenticated;
...
comment on column public.payout_orders.dcc_revenue_usdt is
  'Signed DCC amount: positive company revenue; negative merchant discount or company cost.';
comment on column public.payout_orders.total_company_revenue_usdt is
  'merchant_fee_revenue_usdt plus signed dcc_revenue_usdt.';
...
begin
  ...
  if source_run_id is null then
    raise exception 'DCC_SIGNED_ADDITION_SOURCE_RUN_NOT_FOUND';
  end if;
  if validation.id is null then
    raise exception 'DCC_SIGNED_ADDITION_VALIDATION_RUN_NOT_FOUND';
  end if;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'dcc_treatment', 'SIGNED_AMOUNT_ADDED_ONCE',
      'positive_dcc_meaning', 'COMPANY_REVENUE',
      'negative_dcc_meaning', 'MERCHANT_DISCOUNT_OR_COMPANY_COST',
  ...
  select
    ...
    source.calculation_snapshot || jsonb_build_object(
      ...
      'merchant_fee_rate_denominator', 'MERCHANT_PRINCIPAL_USDT',
      'source_allocation_run_id', source_run_id,
  ...
  insert into public.shadow_pricing_run_supersessions(
    superseded_run_id,
    replacement_run_id,
    reason,
    metadata
  ...
  values (
    source_run_id,
    new_run_id,
    'DCC positive company revenue was incorrectly subtracted.',
    jsonb_build_object(
      'incorrect_formula', 'MERCHANT_FEE_MINUS_DCC',
      'correct_formula', 'MERCHANT_FEE_PLUS_SIGNED_DCC',
      'source_migration', 'fix_dcc_revenue_subtraction_v1',
      'restoration_migration', 'restore_dcc_revenue_addition_v1'
    )
  );
  ...
  values (
    'RESTORE_DCC_SIGNED_ADDITION_SHADOW_RUN',
    'pricing_run',
    new_run_id::text,
    jsonb_build_object(
      'mode', 'SHADOW',
    ...
    jsonb_build_object(
      'superseded_pricing_run_id', source_run_id,
      'per_order_verified_count', validation.payout_exact_match_rows,
      'aggregate_execution_validated_count',
        validation.aggregate_execution_validated_count
    )
...
begin
  ...
  if restored_run_id is null then
    raise exception 'DCC_SIGNED_ADDITION_RUN_NOT_CREATED';
  end if;
  ...
  if exists (
    ...
    raise exception 'DCC_SIGNED_ADDITION_ORDER_ASSERTION_FAILED';
  end if;

  if (
    select count(*)
    from public.payout_profit_calculations
    where pricing_run_id = restored_run_id
  ) <> (
    select count(*)
    from public.payout_orders
  ) then
    raise exception 'DCC_SIGNED_ADDITION_CALCULATION_COUNT_MISMATCH';
  end if;
  ...
  if exists (
    ...
    raise exception 'DCC_SIGNED_ADDITION_SHADOW_ASSERTION_FAILED';
  end if;

  if not exists (
    select 1
    from public.shadow_pricing_run_supersessions supersession
    where supersession.superseded_run_id = wrong_run_id
      and supersession.replacement_run_id = restored_run_id
      and supersession.effective_status = 'SUPERSEDED'
  ) then
    raise exception 'DCC_SUBTRACTION_RUN_NOT_MARKED_SUPERSEDED';
  end if;
  ...
  if (
    ...
    raise exception 'DCC_SIGNED_ADDITION_MARGIN_PARTITION_FAILED';
  end if;
end
$$;
