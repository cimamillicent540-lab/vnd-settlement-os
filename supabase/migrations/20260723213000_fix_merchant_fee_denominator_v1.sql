-- Task 2.5 merchant-fee denominator correction.
-- Raw Payout amounts and prior immutable Shadow runs remain unchanged.
-- DCC is signed revenue and is always added once to merchant fee revenue.
...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'payout_orders'
      and column_name = 'merchant_fee_rate'
  ) and not exists (
    select 1
    from information_schema.columns
    where table_schema = 'public'
      and table_name = 'payout_orders'
      and column_name = 'fee_rate_on_total'
  ) then
    alter table public.payout_orders
      rename column merchant_fee_rate to fee_rate_on_total;
  end if;
end
$$;

alter table public.payout_orders
  add column if not exists merchant_total_debit_usdt numeric(38,8)
    generated always as (received_usdt) stored,
  add column if not exists merchant_principal_usdt numeric(38,8)
    generated always as (
      received_usdt - total_fee_usdt
    ) stored,
  add column if not exists merchant_fee_rate numeric(38,18)
    generated always as (
      total_fee_usdt / nullif(received_usdt - total_fee_usdt, 0)
    ) stored;

comment on column public.payout_orders.amount_usdt is
  'Legacy alias of merchant_total_debit_usdt; source 金额USDT includes fee.';
comment on column public.payout_orders.merchant_total_debit_usdt is
  'Original source 金额USDT: merchant principal plus merchant fee.';
comment on column public.payout_orders.merchant_principal_usdt is
  'Merchant principal: merchant_total_debit_usdt minus merchant_fee_usdt.';
comment on column public.payout_orders.merchant_fee_rate is
  'Formal merchant fee rate: merchant_fee_usdt / merchant_principal_usdt.';
comment on column public.payout_orders.fee_rate_on_total is
  'Diagnostic only: merchant_fee_usdt / merchant_total_debit_usdt.';
comment on column public.payout_orders.dcc_revenue_usdt is
  'Signed DCC amount: positive for revenue, negative for discount or company cost.';
comment on column public.payout_orders.total_company_revenue_usdt is
  'merchant_fee_revenue_usdt + signed dcc_revenue_usdt.';

alter table public.payout_profit_calculations
  add column if not exists merchant_total_debit_usdt numeric(30,8),
  add column if not exists merchant_principal_usdt numeric(30,8),
  add column if not exists fee_rate_on_total numeric(38,18),
  add column if not exists aggregate_execution_validation_status text;

alter table public.payout_profit_calculations
  drop constraint if exists payout_profit_aggregate_validation_check;
alter table public.payout_profit_calculations
  add constraint payout_profit_aggregate_validation_check check (
    aggregate_execution_validation_status is null
    or aggregate_execution_validation_status in (
...
comment on column
  public.payout_profit_calculations.aggregate_execution_validation_status is
  'Run-level Account History evidence; never promotes a Payout to VERIFIED.';

alter table public.task25_validation_runs
  add column if not exists aggregate_execution_validation_status text
    generated always as (
      case
        when successful_unrefunded_rows > 0
          then 'AGGREGATE_EXECUTION_VALIDATED'
        else 'NOT_APPLICABLE'
      end
    ) stored,
  add column if not exists aggregate_execution_validated_count integer
    generated always as (successful_unrefunded_rows) stored;
...
begin
  ...
  if source_run_id is null then
    raise exception 'MERCHANT_FEE_DENOMINATOR_SOURCE_RUN_NOT_FOUND';
  end if;
  if validation.id is null then
    raise exception 'MERCHANT_FEE_DENOMINATOR_VALIDATION_RUN_NOT_FOUND';
  end if;
  ...
  values (
    'HISTORICAL_BACKTEST',
    'SHADOW_PRICING_MERCHANT_FEE_DENOMINATOR_V1',
    jsonb_build_object(
      ...
      'dcc_treatment', 'SIGNED_REVENUE_ADDED_ONCE',
  ...
  select
    ...
    input_snapshot || jsonb_build_object(
      'merchant_fee_denominator_run', true,
      'source_pricing_run_id', source_run_id
    )
  ...
  select
    ...
    source.issue_codes || jsonb_build_array(
      'MERCHANT_FEE_DENOMINATOR_CORRECTED',
      'AGGREGATE_EXECUTION_VALIDATION_NOT_PER_ORDER'
    ),
    source.calculation_snapshot || jsonb_build_object(
      'merchant_total_debit_source', '金额USDT',
      'merchant_principal_formula', 'TOTAL_DEBIT_MINUS_FEE',
      'merchant_fee_rate_denominator', 'MERCHANT_PRINCIPAL_USDT',
      'fee_rate_on_total', payout.fee_rate_on_total,
  ...
  values (
    'CREATE_MERCHANT_FEE_DENOMINATOR_SHADOW_RUN',
    'pricing_run',
    new_run_id,
    jsonb_build_object(
      'mode', 'SHADOW',
      'rules_version', 'SHADOW_PRICING_MERCHANT_FEE_DENOMINATOR_V1',
      'merchant_fee_denominator', 'MERCHANT_PRINCIPAL_USDT',
      'dcc_treatment', 'SIGNED_REVENUE_ADDED_ONCE',
      'automatic_funds_actions', false
    ),
...
begin
  ...
  if corrected_run_id is null then
    raise exception 'MERCHANT_FEE_DENOMINATOR_RUN_NOT_CREATED';
  end if;

  if exists (
    select 1
    from public.payout_orders
    where merchant_principal_usdt is distinct from
        merchant_total_debit_usdt - merchant_fee_usdt
      or abs(
        merchant_fee_rate
          - merchant_fee_usdt / nullif(merchant_principal_usdt, 0)
      ) > 0.000000000000000001
      or abs(
        fee_rate_on_total
          - merchant_fee_usdt / nullif(merchant_total_debit_usdt, 0)
      ) > 0.000000000000000001
      or total_company_revenue_usdt is distinct from
        merchant_fee_usdt + dcc_revenue_usdt
  ) then
    raise exception 'MERCHANT_FEE_DERIVATION_ASSERTION_FAILED';
  end if;
  ...
  if (
    ...
    raise exception 'MERCHANT_FEE_DENOMINATOR_CALCULATION_COUNT_MISMATCH';
  end if;
  ...
  if exists (
    ...
    raise exception 'MERCHANT_FEE_DENOMINATOR_SHADOW_ASSERTION_FAILED';
  end if;
  ...
  if (
    ...
    raise exception 'MARGIN_BUCKET_PARTITION_ASSERTION_FAILED';
  end if;
end
$$;
