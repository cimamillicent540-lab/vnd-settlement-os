...
describe("proportional pool allocation",()=>{
  const buckets:PoolBucketInput[]=[{id:"payin",sourceType:"PAYIN",settleableAvailableAmountVnd:"70000000",grossAvailableAmountVnd:"140000000",costBasisStatus:"MISSING"},{id:"topup",sourceType:"TOPUP",settleableAvailableAmountVnd:"30000000",grossAvailableAmountVnd:"60000000",fundingRateVndPerUsdt:"26000",costBasisStatus:"KNOWN"}];
  ...
  it("assigns the cent residual to the final active bucket",()=>{const result=allocatePayoutProportionally([{...buckets[0],settleableAvailableAmountVnd:"2"},{...buckets[1],settleableAvailableAmountVnd:"1"}],"2");expect(result.map(row=>row.allocatedVnd)).toEqual(["1.33","0.67"])});
...
describe("settleable balance correction",()=>{
  it("derives reserve and settleable opening from the gross upstream balance",()=>expect(deriveBalanceLayers("3398228791.14")).toEqual({grossBalanceVnd:"3398228791.14",reserveRatio:"0.50",reserveAmountVnd:"1699114395.57",settleableRatio:"0.50",settleableBalanceVnd:"1699114395.57"}));
  it("contains no active multiplier=2 rule",()=>expect("accountBalanceMultiplier" in RULES).toBe(false));
  it("keeps original Account History amounts unchanged",()=>{const row=mapAccountHistoryEntry({sourceRowNumber:2,businessOrderNumber:"A",transactionType:"代付",changeAmountVnd:"47473331.73",grossOrderAmountVnd:"47237146.00",feeVnd:"236185.73",direction:"减少",balanceBeforeVnd:"3239911755.00",balanceAfterVnd:"3192438423.00",sourceLocalTime:"2026-07-18 23:59:28"});expect(row.changeAmountVnd).toBe("47473331.73");expect(row.balanceAfterVnd).toBe("3192438423.00")});
  it("derives settleable change as exactly 50% of gross change",()=>expect(deriveSettleableChange("47473331.73")).toBe("23736665.8650"));
  it("checks payout ability against settleable rather than gross balance",()=>{expect(canExecutePayout("100","150")).toBe(false);expect(canExecutePayout("160","150")).toBe(true)});
  it("uses settleable balance for low-pool warning",()=>expect(poolThresholdSnapshot("1000000000","25000")).toEqual({balanceBasis:"SETTLEABLE",equivalentUsdt:"40000.00000000",isLow:true}));
...
describe("quality and privacy controls",()=>{
  ...
  it("records whether a settleable reference-rate snapshot is below 50k USDT",()=>{expect(poolThresholdSnapshot("1000000000","25000").isLow).toBe(true);expect(poolThresholdSnapshot("1500000000","25000").isLow).toBe(false)});
});
